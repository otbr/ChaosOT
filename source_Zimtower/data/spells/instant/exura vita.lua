attackType = ATTACK_NONE
 ...
 function onCast(cid, creaturePos, level, maglv, var)
    ...
    -- n = tonumber(var)   -- try to convert it to a number
    	...
    	-- else
    		--UltimateHealingObject.minDmg = (level * 6 + maglv * 9) * 6
    		--UltimateHealingObject.maxDmg = (level * 6 + maglv * 9) * 6.8
 UltimateHealingObject.minDmg = (level * 6 + maglv * 9) * 6.2
 if UltimateHealingObject.minDmg < 250 then
 	UltimateHealingObject.minDmg = 250
 end
 
 UltimateHealingObject.maxDmg = (level * 6 + maglv * 9) * 9
