  ...
  if lvl >= level then
    if lvl2 >= level2 then
      if waiting(cid, number, days) == -1 then
        ...
        result = rep + 1
        doPlayerSendTextMessage(cid,22,"You gave 1 props point to "..name..", now he/she has "..result.." points.")
        doPlayerSendTextMessage(item2.uid,22,"You've received 1 props point from "..name2..", now you have "..result.." points.")
      else
        doPlayerSendCancel(cid,"You need to wait more time before start to give more props points.") 
      end
    else
      doPlayerSendCancel(cid,"The player need level "..level2.." to receive props points.")    
    end
  else
    doPlayerSendCancel(cid,"You need level "..level.." to give props points.") 
  end
end
else
