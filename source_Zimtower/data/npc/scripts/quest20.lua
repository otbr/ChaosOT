...
conv = 0
 ...
 function onCreatureSay(cid, type, msg)
 	...
 	if msgcontains(msg, 'quest') and focus == cid and getPlayerStorageValue(cid,20020) == -1 then
 		selfSay('Bring me something from Dinix.')
		conv = 1
 		talk_start = os.clock()
	end
 
	if msgcontains(msg, 'tear') and getPlayerStorageValue(cid,20020) == -1 and focus == cid then
		conv = 1
	...
	if msgcontains(msg, 'yes') and conv == 1 and focus == cid then
 		talk_start = os.clock()
		if doPlayerRemoveItem(cid,2346) == 0 then
		buy(cid,2347,1,0)
		setPlayerStorageValue(cid,20020,1)
		conv = 0
		selfSay('Thank you! Take this cookbook to Degex, Degex is fond of Hidden Places is what I heard')
		else
