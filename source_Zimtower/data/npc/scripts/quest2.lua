...
conv = 0
 ...
 function onCreatureSay(cid, type, msg)
 	...
 	if msgcontains(msg, 'quest') and focus == cid and getPlayerStorageValue(cid,20002) == -1 then
 		selfSay('Bring me something from Markus.')
		conv = 1
 		talk_start = os.clock()
	end
 
	if msgcontains(msg, 'purple tome') and getPlayerStorageValue(cid,20002) == -1 and focus == cid then
		conv = 1
	...
	if msgcontains(msg, 'yes') and conv == 1 and focus == cid then
 		talk_start = os.clock()
		if doPlayerRemoveItem(cid,1982) == 0 then
		buy(cid,1984,1,0)
		setPlayerStorageValue(cid,20002,1)
		conv = 0
		selfSay('Thank you! Take this tome to Mazie, Mazie is fond of spider kings is what I heard')
		else
