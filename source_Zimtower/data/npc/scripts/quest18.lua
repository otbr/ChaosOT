...
conv = 0
 ...
 function onCreatureSay(cid, type, msg)
 	...
 	if msgcontains(msg, 'quest') and focus == cid and getPlayerStorageValue(cid,20018) == -1 then
 		selfSay('Bring me something from Coldstin.')
		conv = 1
 		talk_start = os.clock()
	end
 
	if msgcontains(msg, 'piece') and getPlayerStorageValue(cid,20018) == -1 and focus == cid then
		conv = 1
	...
	if msgcontains(msg, 'yes') and conv == 1 and focus == cid then
 		talk_start = os.clock()
		if doPlayerRemoveItem(cid,2341) == 0 then
		buy(cid,2345,1,0)
		setPlayerStorageValue(cid,20018,1)
		conv = 0
		selfSay('Thank you! Take this report to Dinix, Dinix is fond of the creator hunting is what I heard')
		else
