...
conv = 0
 ...
 function onCreatureSay(cid, type, msg)
 	...
 	if msgcontains(msg, 'quest') and focus == cid and getPlayerStorageValue(cid,20009) == -1 then
 		selfSay('Bring me something from Chucky.')
		conv = 1
 		talk_start = os.clock()
	end
 
	if msgcontains(msg, 'genie') and getPlayerStorageValue(cid,20009) == -1 and focus == cid then
		conv = 1
	...
	if msgcontains(msg, 'yes') and conv == 1 and focus == cid then
 		talk_start = os.clock()
		if doPlayerRemoveItem(cid,2063) == 0 then
		buy(cid,2344,1,0)
		setPlayerStorageValue(cid,20009,1)
		conv = 0
		selfSay('Thank you! Take this genie to Trixie, Trixie is fond of Amazons and Valkyries is what I heard')
		else
