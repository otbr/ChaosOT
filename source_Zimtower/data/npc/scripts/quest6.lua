...
conv = 0
 ...
 function onCreatureSay(cid, type, msg)
 	...
 	if msgcontains(msg, 'quest') and focus == cid and getPlayerStorageValue(cid,20006) == -1 then
 		selfSay('Bring me something from Crazie.')
		conv = 1
 		talk_start = os.clock()
	end
 
	if msgcontains(msg, 'map') and getPlayerStorageValue(cid,20006) == -1 and focus == cid then
		conv = 1
	...
	if msgcontains(msg, 'yes') and conv == 1 and focus == cid then
 		talk_start = os.clock()
		if doPlayerRemoveItem(cid,1956) == 0 then
		buy(cid,1968,1,0)
		setPlayerStorageValue(cid,20006,1)
		conv = 0
		selfSay('Thank you! Take this Document to Spencer, Spencer is fond of Garden Grove from what I heard')
		else
