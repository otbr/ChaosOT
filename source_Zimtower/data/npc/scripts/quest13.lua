...
conv = 0
 ...
 function onCreatureSay(cid, type, msg)
 	...
 	if msgcontains(msg, 'quest') and focus == cid and getPlayerStorageValue(cid,20013) == -1 then
 		selfSay('Bring me something from Palus.')
		conv = 1
 		talk_start = os.clock()
	end
 
	if msgcontains(msg, 'piece') and getPlayerStorageValue(cid,20013) == -1 and focus == cid then
		conv = 1
	...
	if msgcontains(msg, 'yes') and conv == 1 and focus == cid then
 		talk_start = os.clock()
		if doPlayerRemoveItem(cid,2336) == 0 then
		buy(cid,2337,1,0)
		setPlayerStorageValue(cid,20013,1)
		conv = 0
		selfSay('Thank you! Take this piece to Moxie, Moxie is fond of Iceland is what I heard')
		else
