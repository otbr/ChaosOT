...
conv = 0
 ...
 function onCreatureSay(cid, type, msg)
 	...
 	if msgcontains(msg, 'quest') and focus == cid and getPlayerStorageValue(cid,20003) == -1 then
 		selfSay('Bring me something from Shazie.')
		conv = 1
 		talk_start = os.clock()
	end
 
	if msgcontains(msg, 'blue tome') and getPlayerStorageValue(cid,20003) == -1 and focus == cid then
		conv = 1
	...
	if msgcontains(msg, 'yes') and conv == 1 and focus == cid then
 		talk_start = os.clock()
		if doPlayerRemoveItem(cid,1984) == 0 then
		buy(cid,1985,1,0)
		setPlayerStorageValue(cid,20003,1)
		conv = 0
		selfSay('Thank you! Take this tome to Dazie, Dazie is fond of Heaven is what I heard')
		else
