...
conv = 0
 ...
 function onCreatureSay(cid, type, msg)
 	...
 	if msgcontains(msg, 'quest') and focus == cid and getPlayerStorageValue(cid,20027) == -1 then
 		selfSay('Bring me a blue robe.')
		conv = 1
 		talk_start = os.clock()
	end
 
	if msgcontains(msg, 'robe') and getPlayerStorageValue(cid,20027) == -1 and focus == cid then
		conv = 1
		selfSay('Could you give it to me, please? I need it badly for the magic wars brewing')
 		talk_start = os.clock()
	end
	if msgcontains(msg, 'yes') and conv == 1 and focus == cid then
		...
		conv = 0
		selfSay('A bold move hero. And a fair trade. Begone now')
		else
