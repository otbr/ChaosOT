...
function onCreatureSay(cid, type, msg)
  	...
  	if (msgcontains(msg, 'hi') and (focus == 0)) and getDistanceToCreature(cid) < 4 then
  		selfSay('Hello ' .. creatureGetName(cid) .. '! I sell crossbows (200gps), bows (100gps), arrows (100gps), bolts (150gps)Enhanced Power Bolts (PB)-(1k per 100) and spears (100gps).')
  		focus = cid
  		talk_start = os.clock()
  	...
  	elseif focus == cid then
		...
		elseif msgcontains(msg, 'spears') then
			buy(cid,2389,10,100)
		elseif msgcontains(msg, 'pb') then
			buy(cid,2547,100,1000)
