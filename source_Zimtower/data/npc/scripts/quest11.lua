...
conv = 0
 ...
 function onCreatureSay(cid, type, msg)
 	...
 	if msgcontains(msg, 'quest') and focus == cid and getPlayerStorageValue(cid,20011) == -1 then
 		selfSay('Bring me something from Trixie.')
		conv = 1
 		talk_start = os.clock()
	end
 
	if msgcontains(msg, 'fanfare') and getPlayerStorageValue(cid,20011) == -1 and focus == cid then
		conv = 1
	...
	if msgcontains(msg, 'yes') and conv == 1 and focus == cid then
 		talk_start = os.clock()
		if doPlayerRemoveItem(cid,2077) == 0 then
		buy(cid,2335,1,0)
		setPlayerStorageValue(cid,20011,1)
		conv = 0
		selfSay('Thank you! Take this piece to palus, palus is fond of mining is what I heard')
		else
