...
conv = 0
 ...
 function onCreatureSay(cid, type, msg)
 	...
 	if msgcontains(msg, 'quest') and focus == cid and getPlayerStorageValue(cid,20019) == -1 then
 		selfSay('Bring me something from Rawr.')
		conv = 1
 		talk_start = os.clock()
	end
 
	if msgcontains(msg, 'report') and getPlayerStorageValue(cid,20019) == -1 and focus == cid then
		conv = 1
	...
	if msgcontains(msg, 'yes') and conv == 1 and focus == cid then
 		talk_start = os.clock()
		if doPlayerRemoveItem(cid,2345) == 0 then
		buy(cid,2346,1,0)
		setPlayerStorageValue(cid,20019,1)
		conv = 0
		selfSay('Thank you! Take this tear to Diphtho, Diphtho is fond of Lizard King is what I heard')
		else
