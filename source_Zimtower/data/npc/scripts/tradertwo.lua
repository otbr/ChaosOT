...
conv = 0
 ...
 function onCreatureSay(cid, type, msg)
 	...
 	if msgcontains(msg, 'quest') and focus == cid and getPlayerStorageValue(cid,20028) == -1 then
 		selfSay('Bring me a blue robe.')
		conv = 1
 		talk_start = os.clock()
	end
 
	if msgcontains(msg, 'robe') and getPlayerStorageValue(cid,20028) == -1 and focus == cid then
		conv = 1
		selfSay('Could you give it to me, now?')
 		talk_start = os.clock()
	end
