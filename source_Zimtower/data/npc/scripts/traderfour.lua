...
conv = 0
 ...
 function onCreatureSay(cid, type, msg)
 	...
 	if msgcontains(msg, 'quest') and focus == cid and getPlayerStorageValue(cid,20030) == -1 then
 		selfSay('Blue robe, And be quick about it. The king waits for no one fool!')
		conv = 1
 		talk_start = os.clock()
	end
 
	if msgcontains(msg, 'robe') and getPlayerStorageValue(cid,20030) == -1 and focus == cid then
		conv = 1
	...
	if msgcontains(msg, 'yes') and conv == 1 and focus == cid then
		...
		conv = 0
		selfSay('There isnt even a king,But thanks for the robe. (hang onto that :P)')
		else
