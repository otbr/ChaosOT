...
conv = 0
 ...
 function onCreatureSay(cid, type, msg)
 	...
 	if msgcontains(msg, 'quest') and focus == cid and getPlayerStorageValue(cid,20024) == -1 then
 		selfSay('Bring me something from Leet.')
		conv = 1
 		talk_start = os.clock()
	end
 
	if msgcontains(msg, 'voodoo doll') and getPlayerStorageValue(cid,20024) == -1 and focus == cid then
		conv = 1
	...
	if msgcontains(msg, 'yes') and conv == 1 and focus == cid then
 		talk_start = os.clock()
		if doPlayerRemoveItem(cid,3955) == 0 then
		buy(cid,2200,1,0)
		setPlayerStorageValue(cid,20024,1)
		conv = 0
		selfSay('OMFG YOU DID IT. YOU PIMP!!! YOU ENJOY YOUR RING NOW AND CONGRATS!!')
		else
