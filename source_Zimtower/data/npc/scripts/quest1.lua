...
conv = 0
 ...
 function onCreatureSay(cid, type, msg)
 	...
 	if msgcontains(msg, 'quest') and focus == cid and getPlayerStorageValue(cid,20001) == -1 then
 		selfSay('Bring me something from the temple of chaos city.')
		conv = 1
 		talk_start = os.clock()
	end
 
	if msgcontains(msg, 'green tome') and getPlayerStorageValue(cid,20001) == -1 and focus == cid then
		conv = 1
	...
	if msgcontains(msg, 'yes') and conv == 1 and focus == cid then
 		talk_start = os.clock()
		if doPlayerRemoveItem(cid,1983) == 0 then
		buy(cid,1982,1,0)
		setPlayerStorageValue(cid,20001,1)
		conv = 0
		selfSay('Thank you! Take this tome to shazie, Shazie is fond of dragons is what I heard')
		else
