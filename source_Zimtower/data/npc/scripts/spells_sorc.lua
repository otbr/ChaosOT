-- NPC Gambler by Alreth, v1.1, 2006-06-16 20:03 CET
		...
		elseif ((string.find(msg, '(%a*)hi(%a*)') or string.find(msg, '(%a*)hello(%a*)')) and (focus ~= cid) and getDistanceToCreature(cid) < 4)  then
	 	...
	 	elseif (focus == cid) then
			...
			if (msgcontains(msg, 'dice') or msgcontains(msg, 'game') or msgcontains(msg, 'gamble')) then
				if (getPlayerLevel(creatureGetName(cid)) >= 40) then
				...
				if (tonumber(msg) <= 0 or tonumber(msg) > maxBid) then
					...
					if (pay(cid, bid)) then
						...
						if (dice == 6) then
                	        ...
                	        if ((bid*5) >= broadcastWin) then
                	            selfSay('/B '..creatureGetName(cid)..' just won '..(bid*5)..' gold from gambling!')
							end
							inConvo = 1
						else
...
function onThink()
 	if (os.clock() - talk_start) > 30 then
        if (focus > 0) then	
    		...
    		if (next_focus > 0 and distFocus ~= nil and distFocus < 4) then
    			selfSay('Greetings, high roller. So you care for a game,  ' .. creatureGetName(next_focus) .. '?')
    			talk_start = os.clock()
    			focus = next_focus
    			next_focus = 0
