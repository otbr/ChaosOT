...
conv = 0
 ...
 function onCreatureSay(cid, type, msg)
 	...
 	if msgcontains(msg, 'quest') and focus == cid and getPlayerStorageValue(cid,20005) == -1 then
 		selfSay('Bring me something from Dazie')
		conv = 1
 		talk_start = os.clock()
	end
 
	if msgcontains(msg, 'red tome') and getPlayerStorageValue(cid,20005) == -1 and focus == cid then
		conv = 1
	...
	if msgcontains(msg, 'yes') and conv == 1 and focus == cid then
 		talk_start = os.clock()
		if doPlayerRemoveItem(cid,1986) == 0 then
		buy(cid,1956,1,0)
		setPlayerStorageValue(cid,20005,1)
		conv = 0
		selfSay('Thank you! Take this map to Lazie, Lazie is fond of the desert is what I heard')
		else
