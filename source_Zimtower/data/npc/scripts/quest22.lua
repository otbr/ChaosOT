...
conv = 0
 ...
 function onCreatureSay(cid, type, msg)
 	...
 	if msgcontains(msg, 'quest') and focus == cid and getPlayerStorageValue(cid,20022) == -1 then
 		selfSay('Bring me something from Degex.')
		conv = 1
 		talk_start = os.clock()
	end
 
	if msgcontains(msg, 'chocolate') and getPlayerStorageValue(cid,20022) == -1 and focus == cid then
		conv = 1
	...
	if msgcontains(msg, 'yes') and conv == 1 and focus == cid then
 		talk_start = os.clock()
		if doPlayerRemoveItem(cid,2355) == 0 then
		buy(cid,2361,1,0)
		setPlayerStorageValue(cid,20022,1)
		conv = 0
		selfSay('Thank you! Take this star light to leet, leet is fond of Hidden places is what I heard')
		else
