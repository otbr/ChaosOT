...
conv = 0
 ...
 function onCreatureSay(cid, type, msg)
 	...
 	if msgcontains(msg, 'quest') and focus == cid and getPlayerStorageValue(cid,20021) == -1 then
 		selfSay('Bring me something from Diphtho.')
		conv = 1
 		talk_start = os.clock()
	end
 
	if msgcontains(msg, 'cookbook') and getPlayerStorageValue(cid,20021) == -1 and focus == cid then
		conv = 1
	...
	if msgcontains(msg, 'yes') and conv == 1 and focus == cid then
 		talk_start = os.clock()
		if doPlayerRemoveItem(cid,2347) == 0 then
		buy(cid,2355,1,0)
		setPlayerStorageValue(cid,20021,1)
		conv = 0
		selfSay('Thank you! Take this chocolate to Roxor, Roxor is fond of hidden places is what I heard')
		else
