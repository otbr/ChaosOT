...
conv = 0
 ...
 function onCreatureSay(cid, type, msg)
 	...
 	if msgcontains(msg, 'quest') and focus == cid and getPlayerStorageValue(cid,20007) == -1 then
 		selfSay('Bring me something from Lazie.')
		conv = 1
 		talk_start = os.clock()
	end
 
	if msgcontains(msg, 'document') and getPlayerStorageValue(cid,20007) == -1 and focus == cid then
		conv = 1
	...
	if msgcontains(msg, 'yes') and conv == 1 and focus == cid then
 		talk_start = os.clock()
		if doPlayerRemoveItem(cid,1968) == 0 then
		buy(cid,2062,1,0)
		setPlayerStorageValue(cid,20007,1)
		conv = 0
		selfSay('Thank you! Take this Genie to Chucky, Chucky is fond of Chaos City is what I heard')
		else
